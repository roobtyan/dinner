				...
				<ul style=" display:inline-table">
					<c:choose>
						<c:when test="${not empty requestScope.dinnerTables}">
							<c:forEach items="${requestScope.dinnerTables}" var="dinnerTable">
								<li>
									<a href="${pageContext.request.contextPath}/food?method=foodDetail&tableId=${dinnerTable.id}">
						...
						<c:otherwise>
							<p style="font-size: 20px;color: red">对不起，当前没有空余餐桌，请稍等。。。。</p>
						</c:otherwise>
					</c:choose>
				</ul>
			</div>
